...
class YOLOLoss(nn.Module):
    ...
    def forward(self, input, targets=None):
        ...
        if self.GPU:
            ...
            tconf, tcls = tconf, tcls.to(self.device)
    ...
    def get_ignore(self,prediction,target,scaled_anchors,in_w, in_h,noobj_mask):
        ...
        grid_x = torch.linspace(0, in_w - 1, in_w).repeat(in_h, 1).repeat(
            int(bs*self.num_anchors/3), 1, 1).view(x.shape).to(self.device).to(dtype=torch.float)
        grid_y = torch.linspace(0, in_h - 1, in_h).repeat(in_w, 1).t().repeat(
            int(bs*self.num_anchors/3), 1, 1).view(y.shape).to(self.device).to(dtype=torch.float)

        # 生成先验框的宽高
        anchor_w = torch.tensor(scaled_anchors,dtype=torch.float).to(self.device).index_select(1,  torch.tensor([0], dtype=torch.long, device=self.device))
        anchor_h = torch.tensor(scaled_anchors,dtype=torch.float).to(self.device).index_select(1,  torch.tensor([1], dtype=torch.long, device=self.device))
        ...
        #   计算调整后的先验框中心与宽高
        #-------------------------------------------------------#
        pred_boxes = torch.zeros(prediction[..., :4].shape, dtype=torch.float, device=self.device)  # 这里初始化 pred_boxes
        ...
        for i in range(bs):
            ...
            if len(target[i]) > 0:
                ...
                gt_box = torch.cat([gx, gy, gw, gh],-1).to(self.device)
